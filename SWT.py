...
class SWTForward(nn.Module):
    ...
    def forward(self, x):
        ...
        Args:
            x: input shape (N)
        Return_: [b, 2*4, h, w], 2 for complex-value
        ...
        filts = tuple(item.to(ll.device) for item in filts)
...
class SWTInverse(nn.Module):
    ...
    """
    def __init__(self, wave='haar', mode='symmetric'):
        super().__init__()
        if isinstance(wave, str):
            wave = pywt.Wavelet(wave)
        if isinstance(wave, pywt.Wavelet):
    ...
    def forward(self, coeffs):
        ...
        Args: [1, 2*4, h, w] 2 for complex-value
        """
        ...
        ll_r, lh_r, hl_r, hh_r, ll_i, lh_i, hl_i, hh_i = torch.unbind(coeffs, dim=1)
        
        filts = (self.g0_col, self.g1_col, self.g0_row, self.g1_row)
        filts = tuple(item.to(coeffs.device) for item in filts)

        recon_r = sfb2d_atrous(ll_r, lh_r, hl_r, hh_r, filts, mode=self.mode)
        recon_i = sfb2d_atrous(ll_i, lh_i, hl_i, hh_i, filts, mode=self.mode)
            
        return torch.cat([recon_r, recon_i], dim=1)
